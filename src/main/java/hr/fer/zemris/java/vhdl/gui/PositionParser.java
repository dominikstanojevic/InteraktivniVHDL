...
public class PositionParser {
    public enum Position {
        ...
        public static Optional<Position> getValueFromString(String string) {
            return Arrays.stream(Position.values()).filter(p -> p.string.equals(string)).findFirst();
        }
    }
    ...
    private void parseLine() {
        ...
        Declaration s = table.getUut().getPort(definition.signal);
        if (definition.start == null && s.getType() == Type.VECTOR_STD_LOGIC) {
            VectorData data = s.getVectorData();
            definition.start = data.getStart();
            definition.end = data.getOrder() == VectorOrder.TO ?
                    definition.start + data.getSize() - 1 :
                    definition.start - data.getSize() + 1;
        }
    ...
    private void parseParentheses(Definition definition) {
        ...
        if (lexer.getCurrentToken().getType() == TokenType.CLOSED_PARENTHESES) {
            ...
            VectorOrder order = getOrder(definition, (String) currentValue());
            lexer.nextToken();
    ...
    private void checkOrder(String signal, int first, VectorOrder order, int second) {
        if (order == VectorOrder.TO && first <= second) {
            return;
        }
        if (order == VectorOrder.DOWNTO && first >= second) {
            return;
        }
    ...
    private VectorOrder getOrder(Definition definition, String order) {
        switch (order) {
            case "to":
                return VectorOrder.TO;
            case "downto":
                return VectorOrder.DOWNTO;
            default:
                throw new ParserException("Invalid order for signal " + definition.signal + ".");
        }
    }
